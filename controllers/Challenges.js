...
export const createQuiz = async (req, res) => {
  try {
    ...
    await newQuiz.save().then((docs) => {
      recruiterModel
        .updateOne({ _id: data.createdBy }, { $push: { quizs: docs._id } })
        .then(() => {
  ...
  try {
    ...
    if (data.questions) {
      for (var i = 0; i < data.questions.length; i++) {
        var time =
          data.questions[i].duration.minutes +
          data.questions[i].duration.seconds / 60;
        totalDuration += time;
      }
    ...
    recruiterModel.findById(userId, async (err, result) => {
      if (err) {
        ...
        if (!result?.quizs) {
          res
            .status(403)
            .json({ message: "No quizs are created by this user" });
        } else {
          questionModel
            .find()
            .where("_id")
...
export const getQuizsOfUserLazyLoading = async (req, res) => {
  try {
    ...
    const quizs = await questionModel
