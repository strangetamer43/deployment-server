...
export const registerTask = async (req, res) => {
  ...
  if (!index) {
    ...
    task.registrations = task.registrations.filter(
      task.registrations.user !== String(req.userId)
    );
  }
  const updatedTask = await TaskMessage.findByIdAndUpdate(id, task, {
    new: true,
  });
  res.json(updatedTask);
};
...
export const completeTask = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))
    return res.status(404).send("No task with that ID");
  const task = await TaskMessage.findById(id);
  const index = task.completions.user.findIndex(
  ...
  if (index === -1) {
    ...
    task.completions = task.completions.user.filter(
      (id) => id !== String(req.userId)
    );
  }
  const updatedTask = await TaskMessage.findByIdAndUpdate(id, task, {
